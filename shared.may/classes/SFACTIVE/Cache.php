<?php // vim:et:ai:ts=4:st=4:sw=4
namespace SFACTIVE;

/**
 * This class is deprecated. Use a standard cache syste and recode toward that.
 */
...
class Cache
{
	...
	if(!$_POST['language_id'])
          ...
          if (($_POST["file_count"] <= 20))
          {
               ...
               for ($counter = 1; $_POST["file_count"] >= $counter; $counter++)
               {
                        ...
                        ($_FILES["linked_file_".$counter]["name"] != "none"))
                    {
                         ...
                         $clean_basename = strtolower(preg_replace("/[^a-zA-Z0-9._-]/", "_", $_FILES["linked_file_".$counter]["name"]));
                         $upload_target = $GLOBALS["upload_folder"] . $clean_basename;
                         preg_match('#([a-z0-9/\._-]+)\.([a-z0-9]+)$#i', $upload_target, $regs);
                         $upload_ext = $regs[2];
     ...
     function process_uploads()
     {
		...
		// holy shit, must rewrite this logic -johnk
               if (is_uploaded_file($tmpfilename = $_FILES["linked_file_".$counter]["tmp_name"]))
               {
                    ...
                    $clean_basename = preg_replace("/[^a-zA-Z0-9._-]/","_", $_FILES["linked_file_".$counter]["name"]);
     ...
     function cache_ram_file($upload_target, $year, $month)
     {
          ...
          $ram_filename=preg_replace("/\.r[avm]$/i",'.ram',basename($upload_target));
...
function MakeCacheDir($ts_date)
{
     ...
     if (preg_match("/([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})/", $ts_date))
     {
