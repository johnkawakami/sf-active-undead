...
class LocalPage extends Cache
{
    // The Page class is used to compile and build pages

    function LocalPage($pageid = '')
    {
	...
	$tr->create_translate_table('page');
        	...
        	} else
        	{
            	...
            	} else
            	{
                	...
                	$this->script_file = SF_LOCAL_PATH . "/classes/pages/" . $pageid . ".inc";
    ...
    function build_page($content_page = '')
    {
        ...
        unset ($this->localpage->forced_template_file);
	
        if (strlen($content_page) > 0 && $this->pageid == "content_page")
        {
            $this->localpage->execute($content_page);
        } 
			else
        {
            $this->localpage->execute();
        }

        if (isset($this->localpage->forced_template_file))
        {
            $this->force_new_template($this->localpage->forced_template_file);
        }
        ...
        if (is_array($this->localpage->tkeys))
        {
            while(list($key, $value) = each($this->localpage->tkeys))
            {
                $keyid = "TPL_" . strtoupper($key);
