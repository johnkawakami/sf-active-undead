...
my $city = $ARGV[0];
my $uploads_dir = "/www/sf-active/$city/website/uploads";
my $dsn = "dbi:Pg:dbname=active_$city";
my $user = 'php';
my $pass = 'fr0gg3r';
...
my $dbh = DBI->connect($dsn, $user, $pass) or die "Cant connect to the DB: $DBI::errstr\n";

my $sth = $dbh->prepare( "SELECT * FROM webcast" ) or die "Can't prepare SQL statement: ", $dbh->errstr(), "\n"; 
...
while ( my @fields = $sth->fetchrow_array() ) {
  ...
  if ($fields[14] =~ /^\S/) {
    ...
    if (! open INFILE, "$uploads_dir/$basename") {
      print STDERR "$uploads_dir/$basename: $!\n";
  ...
  $fields[15] =~ /(\d\d\d\d)-(\d\d)-(\d\d) (\d\d):(\d\d):(\d\d)-(\d\d)/ or die "Malformed date string at id=".$fields[0];
  $fields[15] = $1.$2.$3.$4.$5.$6;
  $fields[3] = "$1-$2-$3 $4:$5:$6";
  $fields[16] =~ /(\d\d\d\d)-(\d\d)-(\d\d) (\d\d):(\d\d):(\d\d)-(\d\d)/ or die "Malformed date string at id=".$fields[0];
  ...
  if ($fields[17] ne '') {
    ...
    $fields[17] = "$uploads_dir/$basename";
  }
