...
/*
  probably should do:
   - break out into a data model and a controller
   - add hooks for after it's loaded, before it's saved: 
   - in configs, do this:
     $articleManager = new ArticleManager();
     $articleManager->registerAfterLoadedFilter($callable)
     $articleManager->registerBeforeSaveFilter($callable)
     $articleManager->registerAfterSaveFilter($callable)
	 and stack up the spam filters, html/txt filters, caching, php generators
	 json serializer, post hider, etc.
	 The filters get an ArticleModel as input, and return a modified version.

	 set mime type can be done as a filter, too.

	 There should also be a "DelayedAfterSave" event as well, where
	 the callback isn't called immediately, but queued.  For example,
	 if an article is updated, the newswire needs to be rebuilt, but
	 it's possible that several articles are being processed in a 
	 batch.  You don't want the newswire rendered every time.
	 THe format for this callback is different. It should be a 
	 static method that takes positional arguments. These can 
	 be queued up in the database as callback, arg1, arg2, arg3.
	 Duplicate rows wouldn't be allowed.  So if you want to
	 regen the newswire, maybe you save: "Newswire::render", 3
	 You an even request to regenerate more than one Newswire.
	 Then, later, the code that's manipulating the batch of articles
	 would call Article::callDelayedAfterSave() and that would
	 process all the pending callbacks. You can even use a 
	 transaction to lock the table, so if two people are using
	 the site, their changes will be processed together.
 */
class Article extends Cache
{
	...
	function update_webcast_indexes() {
		...
		if ($d=='l') {
			$db_obj->execute_statement("DELETE FROM webcast_parents_t WHERE id=$id");
			$db_obj->execute_statement("INSERT INTO webcast_parents_l SELECT * FROM webcast WHERE id=$id");
			$db_obj->execute_statement("INSERT INTO webcast_parents SELECT * FROM webcast WHERE id=$id ON DUPLICATE KEYS UPDATE webcast_parents.display=webcast.display");
		}
		if ($d=='t') {
			$db_obj->execute_statement("DELETE FROM webcast_parents_l WHERE id=$id");
			$db_obj->execute_statement("INSERT INTO webcast_parents_t SELECT * FROM webcast WHERE id=$id");
			$db_obj->execute_statement("INSERT INTO webcast_parents SELECT * FROM webcast WHERE id=$id ON DUPLICATE KEYS UPDATE webcast_parents.display=webcast.display");
		}
		if ($d=='f') {
			$db_obj->execute_statement("DELETE FROM webcast_parents_l WHERE id=$id");
			$db_obj->execute_statement("DELETE FROM webcast_parents_t WHERE id=$id");
			$db_obj->execute_statement("DELETE FROM webcast_parents WHERE id=$id");
		}
	}
	function dump_webcast_latest() 
	{
		...
		*/
		...
		$db_obj = new DB;
		$db_obj->execute_statement("START TRANSACTION");
		$db_obj->execute_statement("DROP TABLE webcast_latest");
		$db_obj->execute_statement("CREATE TABLE webcast_latest SELECT * FROM `webcast` ORDER BY id DESC LIMIT 1000");
		$db_obj->execute_statement("COMMIT");
		$this->has_dumped_webcast_latest = TRUE;
			//var_dump(debug_backtrace());
		//echo "<p>dumped webcast_latest</p>";
	}
	/**
	 * Getting an article and a thread are quick, but rendering them is slow.
	 * Not sure why.  But by dumping these to a smaller table, mabye we can gain speed.
	 * Never mind - this doesn't really work because execute_statement doesn't seem
	 * to work with transactions.  The statements happen, but the select on the table
	 * happens before the inserts complete.
	 */
	function dump_article($id) 
	{
		...
		if (! $this->is_in_webcast_latest($id)) 
		{
			$db_obj = new DB;
			$db_obj->execute_statement("START TRANSACTION");
			$db_obj->execute_statement("INSERT INTO webcast_latest SELECT * FROM webcast WHERE id=$id");
			$db_obj->execute_statement("INSERT INTO webcast_latest SELECT * FROM webcast WHERE parent_id=$id");
			$db_obj->execute_statement("COMMIT");
		}
	}
    ...
    function set_article_data ($article_id = 0)
    {
        ...
        {
			die("Article called with a bad id");
            return 0;
        }
    }
...
// build the comment box where is needed . new config file entry to do it added by blicero
		...
		$htmlheader = "\xEF\xBB\xBF<!doctype html>\r<html><head>\r<meta charset=\"utf-8\" />\r<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" />\r<title>$title</title><link rel=\"stylesheet\" href=\"/css/style.css\"></link></head><body>";
    ...
    function cleanup_text($tmpvar)
    {
        ...
        $tmpvar = eregi_replace("(\r\r \r\r|\n\n \n\n|\r\n\r\n \r\n\r\n|\n\n|\r\r|\r\n\r\n|\n\t|\r\t\|\r\n\t)", " <br /><br /> ", $tmpvar);
        $tmpvar = eregi_replace("(\n|\r|\r\n)", " <br /> ", $tmpvar);

        return $tmpvar;
    }

	function link_text1($tmpvar) 
	{
        // encode
